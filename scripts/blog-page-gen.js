...
(function () {
    ...
    var inject_HTML_page = function (h_page, target) {
        ...
        if (h_doc !== null) {
            ...
            var _body = h_doc.getElementById ('abstract');
            //if (_body[0]) {
            if (_body) {
                // Inject the body
                //target.innerHTML = xml_node_value (_body[0]);
                target.innerHTML = xml_node_value (_body);
                ...
                dbg_trace ('Abstract injected OK');
            } else {
                dbg_error ('No Abstract ID found');
            }
    ...
    var targ_source_attr = 'data-bp-src';
